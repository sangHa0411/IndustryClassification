...
class RobertaArcface(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.W = nn.Parameter(torch.rand(config.hidden_size, config.num_labels), requires_grad=True)
    ...
    def forward(
        ...
        if labels is not None:
            loss_fct = nn.CrossEntropyLoss()
            logits = self.classify(hidden_states, labels)
            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
        else :
            logits = torch.matmul(hidden_states, self.W)
    ...
    def classify(self, hidden_states, labels) :
        x = F.normalize(hidden_states)
        w = F.normalize(self.W)

        fc7 = torch.matmul(x, w)
        original_targets = torch.clamp(fc7, min=-1.0 + 1e-6, max=1.0 - 1e-6)
        theta = torch.acos(original_targets)
        marginal_targets = torch.cos(theta + 0.5)

        labels_one_hot = F.one_hot(labels, num_classes=self.config.num_labels)

        logits = fc7 + labels_one_hot * (marginal_targets - original_targets)
        logits *= 30
        return logits
